...
func openAt(path string) (*DB, error) {
	...
	db.Exec(`ALTER TABLE settings ADD COLUMN show_hints INTEGER NOT NULL DEFAULT 1`)
...
func (d *DB) LoadSettings() (*settings.Settings, error) {
	...
	var showQuotes, discordRPC, autoplayEnabled, showHints, vizOn, lyricsOn int
	row := d.QueryRow(`SELECT playback_mode, default_volume, search_limit, download_dir, download_format, show_quotes, discord_rpc_enabled, autoplay_enabled, theme, show_hints, visualizer_on, lyrics_on FROM settings WHERE id = 1`)
	if err := row.Scan(&s.PlaybackMode, &s.DefaultVolume, &s.SearchLimit, &s.DownloadDir, &s.DownloadFormat, &showQuotes, &discordRPC, &autoplayEnabled, &s.Theme, &showHints, &vizOn, &lyricsOn); err != nil {
		return settings.Defaults(), fmt.Errorf("load settings: %w", err)
	}
	s.ShowQuotes = showQuotes != 0
	s.ShowHints = showHints != 0
	s.DiscordRPCEnabled = discordRPC != 0
	s.AutoplayEnabled = autoplayEnabled != 0
	s.VisualizerOn = vizOn != 0
	s.LyricsOn = lyricsOn != 0
	return &s, nil
}
...
func (d *DB) SaveSettings(s *settings.Settings) error {
	_, err := d.Exec(
		`UPDATE settings SET playback_mode = ?, default_volume = ?, search_limit = ?, download_dir = ?, download_format = ?, show_quotes = ?, discord_rpc_enabled = ?, autoplay_enabled = ?, theme = ?, show_hints = ?, visualizer_on = ?, lyrics_on = ? WHERE id = 1`,
		s.PlaybackMode, s.DefaultVolume, s.SearchLimit, s.DownloadDir, s.DownloadFormat, boolInt(s.ShowQuotes), boolInt(s.DiscordRPCEnabled), boolInt(s.AutoplayEnabled), s.Theme, boolInt(s.ShowHints), boolInt(s.VisualizerOn), boolInt(s.LyricsOn),
	)
	if err != nil {
