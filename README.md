...
- **Favorites, history, library** — `f` to bookmark, a listening-history page, and a filterable Library of everything on disk — your downloads, plus your own music folders — Enter on **Library Folders** in Settings opens a browser, space adds the folder you're in. Tags are read when a file has them; when it doesn't, the filename is the title.
