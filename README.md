...
- **Favorites, history, library** — `f` to bookmark, a listening-history page, and a filterable Library of everything on disk.
- **Play your own music** — Point ytmgo at your music folders and they appear in the Library beside your downloads, playable like anything else. Settings → **Library Folders** → Enter opens a folder browser; space adds the folder you're in. Subfolders are included. Tags are read when a file has them; when it doesn't, the filename is the title — no guessing.
